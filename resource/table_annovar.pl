...
our ($queryfile, $dbloc);
our ($outfile, $buildver, $remove, $checkfile, $protocol, $operation, $otherinfo, $onetranscript, $nastring, $genericdbfile, $gff3dbfile, $bedfile, $vcfdbfile, $csvout, $argument, $tempdir, $vcfinput, $dot2underline, $thread, $maxgenethread, $polishgene, $xreffile,$splicing_threshold);
our $orig_command = $0;
...
GetOptions('verbose|v'=>\$verbose, 'help|h'=>\$help, 'man|m'=>\$man, 'outfile=s'=>\$outfile, 'buildver=s'=>\$buildver, 'remove'=>\$remove, 'checkfile!'=>\$checkfile, 
	...
	'thread=i'=>\$thread, 'maxgenethread=i'=>\$maxgenethread, 'polishgene'=>\$polishgene, 'xreffile=s'=>\$xreffile,'splicing_threshold=i'=>\$splicing_threshold) or pod2usage ();
...
sub geneOperation {
	...
	#generate gene anno
	my $sc;
	#set hgvs and set ±2 boundary: exonicsplicing as default
	#add splicing threshold, set off exonicsplicing tag
	#modified by berry 2017.12.21 yinjie149@berrygenomics.com
	if($splicing_threshold){
		$sc = "annotate_variation.pl -geneanno -buildver $buildver -dbtype $protocol -outfile $tempfile.$protocol --hgvs --splicing_threshold $splicing_threshold -exonsort $queryfile $dbloc";
	}
	else{
		$sc = "annotate_variation.pl -geneanno -buildver $buildver -dbtype $protocol -outfile $tempfile.$protocol --hgvs --exonicsplicing -exonsort $queryfile $dbloc";
	}
	...
	if ($polishgene) {
		if (rename ($e_anno_outfile, "$e_anno_outfile.orig")) {
			$sc = "coding_change.pl $e_anno_outfile.orig $dbloc/${buildver}_$protocol.txt $dbloc/${buildver}_${protocol}Mrna.fa -alltranscript -out $tempfile.$protocol.fa -newevf $e_anno_outfile";
			print STDERR "\nNOTICE: Running with system command <$sc>\n";
			system ($sc) and die "Error running system command: <$sc>\n";
...
=head1 SYNOPSIS
 ...
 Optional arguments:
        ...
        -v, --verbose                   use verbose output
            ...
            --splicing_threshold <int>	specify the threshold to annotate splicing site (default: 2)
...
specify the splicing threshold
